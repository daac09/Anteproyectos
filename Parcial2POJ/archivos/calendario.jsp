            ...
            <div class="mask" style="background-color: rgba(0, 0, 0, 0.7);">
                <div class="d-flex justify-content-center align-items-center h-100">
                    <div class="text-white">
                        ...
                        <h5 class="mb-4">Calendario Academico</h5>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="container-xl">
    	<form class="bg-white formulario border border-primary border-4 shadow-lg p-5 mb-5 rounded" method="post">
            <div class="form-group">
			    <label for="link" class="form-label">Url del calendario academico: </label>
        ...
        <a class="btn btn-primary" href="${fila2.archivo}" target="_blank" role="button">Ver</a>
        </c:forEach>
        <button type="button" class="btn btn-dark" onclick="window.location.href='../administrador/admin.jsp'">Volver</button>
        </form>
    </c:if>
    </div>
    <c:if test="${param.link != null}">
        <sql:update var="result" dataSource="${parcial}">
           UPDATE calendario SET `archivo`='${param.link}';
        </sql:update>
        <c:if test="${result == 1}">
            <script>
            	window.alert("URL actualizada correctamente");
