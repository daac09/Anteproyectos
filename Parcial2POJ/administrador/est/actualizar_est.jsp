...
<body>
    ...
    <h2><center>Actualizar Estudiante</center></h2>
    <c:if test="${param.modifica == null}">
    <sql:query var="estudiantes" dataSource="${parcial}">
        Select * from estudiantes where id = ${param.id}
    </sql:query>
    <div class="container">
    <c:forEach var="items" items="${estudiantes.rows}">
    	<form class="bg-white formulario border border-primary border-4 shadow-lg p-5 mb-5 rounded" method="post">
            <div class="form-group">
			    <label for="id_user" class="form-label">ID de Estudiante: </label>
            ...
            <div class="form-group">
			    ...
			    <input type="text" class="form-control" name="prog" value="${items.programa}" required>
			</div>
            <div class="form-group">
			    <label for="sem" class="form-label">Semestre: </label>
			    <input type="text" class="form-control" name="sem" value="${items.semestre}" required>
			</div>
            <div class="form-group">
			    <label for="fecha_nac" class="form-label">Fecha de nacimiento: </label>
			    <input type="date" max="2001-12-30" class="form-control" name="fecha_nac" value="${items.fecha_nac}" required>
			</div>
        <br>
