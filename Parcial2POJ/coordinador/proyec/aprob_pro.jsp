...
<body>
    ...
    <c:if test="${param.id != null}">
       <%
        ...
        numero_aleatorio=1+rand.nextInt(100000);    
        %>
        <c:set var = "num" value = "<%=numero_aleatorio%>"/>
        <sql:update var="result" dataSource="${parcial}">
            update proyectos
            set apro = 'N° ${num}',
            estado = 'Idea Aprobada'
            where id_pro = ${param.id}
        </sql:update>
        <c:if test="${result == 1}">
        <script>
		    window.alert("Proyecto aprobado correctamente");
            window.history.back();
	    </script>
        </c:if>
        <c:if test="${result == 0}">
